...
/**
 * Get available chat users (patients for clinicians, clinicians for patients)
 */
export const getChatUsers = async (): Promise<RocketChatUser[]> => {
  const token = await SecureStore.getItemAsync('auth_token');
  if (!token) {
    throw new Error('Not authenticated');
  }

  const response = await fetch(API_ENDPOINTS.ROCKETCHAT.USERS, {
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
  ...
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.error || 'Failed to get chat users');
  }
