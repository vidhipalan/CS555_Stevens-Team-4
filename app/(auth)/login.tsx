...
export default function Login() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (!savedEmail || !savedPassword) return setServerError('No account found. Please create an account.');
    if (values.email !== savedEmail || values.password !== savedPassword) return setServerError('Invalid email or password');

    await SecureStore.setItemAsync('auth_token', 'demo_token');
    router.replace('/(tabs)');
  };
    ...
    <KeyboardAvoidingView behavior={Platform.select({ ios: 'padding', android: undefined })} style={{ flex: 1 }}>
      <View style={{ flex: 1, padding: 24, gap: 16, justifyContent: 'center' }}>
        ...
        <Text style={{ marginTop: 12, textAlign: 'center' }}>
          No account? <Link href="/(auth)/signup" style={{ fontWeight: '700' }}>Create one</Link>
        </Text>
      </View>
